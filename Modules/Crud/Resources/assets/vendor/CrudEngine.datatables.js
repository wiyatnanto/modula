(function($) {
    ...
    $.fn.CrudEngineDtable = function( options ) {
     ...
     //   return this.each( function() {
			...
			$('.Action_Row').click(function () {
				...
				if( code =='create') {
			       		var url = settings.action + '?task=create';			       		
			       		var mode = $(this).attr('mode');
						var title = $(this).attr('data-original-title');
						if(mode =='native')
						{
							CrudEngine_ViewDetail( settings.tableId , url  );
						} else {
		        ...
		        if(rows)
		        {
					...
					if(code =='view')
					{
						var url =  settings.action + '?task=view&id='+id;
						var mode = $(this).attr('mode');
						var title = $(this).attr('data-original-title');
						if(mode =='native')
						{
			        	...
			        	if(confirm('Are sure Clone/Copy selected row(s) ?'))
			        	{
							...
							$.post( url  ,{ids:ss, task:"delete", _token: settings._token},function( data ) {
			       		...
			       		var url = settings.action + '?task=update&id='+id;			       		
			       		var mode = $(this).attr('mode');
						var title = $(this).attr('data-original-title');
						if(mode =='native')
						{
