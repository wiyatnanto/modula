...
class BuilderController extends Controller
{
    ...
    function getDestroy($id = null)
    {
        ...
        if (count($row) <= 0) {
            return redirect('crud/builder')
                ->with('message', 'Can not find module')
                ->with('status', 'error');
        }
        ...
        if ($row->type != 'core') {
            if ($class != '') {
                ...
                if (
                    file_exists(
                        app_path() . "/Http/Controllers/{$class}Controller.php"
                    )
                ) {
                    unlink(
                        app_path() . "/Http/Controllers/{$class}Controller.php"
                ...
                return redirect('crud/builder')
                    ->with('message', 'Module has been removed successfull')
                    ->with('status', 'success');
            }
        }
        return redirect($this->module)
            ->with('message', 'No Module removed !')
            ->with('status', 'success');
    }
    ...
    function getConfig($id)
    {
        ...
        $this->data['setting'] = [
            'gridtype' => isset($config['setting'])
                ? $config['setting']['gridtype']
                : 'native',
            'orderby' => isset($config['setting'])
                ? $config['setting']['orderby']
                : $row->db_key,
            'ordertype' => isset($config['setting'])
                ? $config['setting']['ordertype']
                : 'asc',
            'perpage' => isset($config['setting'])
                ? $config['setting']['perpage']
                : '10',
            'frozen' => isset($config['setting']['frozen'])
                ? $config['setting']['frozen']
                : 'false',
            'form-method' => isset($config['setting']['form-method'])
                ? $config['setting']['form-method']
                : 'native',
            'view-method' => isset($config['setting']['view-method'])
                ? $config['setting']['view-method']
                : 'native',
            'inline' => isset($config['setting']['inline'])
                ? $config['setting']['inline']
                : 'false',
        ];
    ...
    function postSaveconfig(Request $request)
    {
        ...
        if ($validator->passes()) {
            ...
            foreach ($lang as $l) {
                if ($l['folder'] != 'en') {
                    $label_lang = isset($_POST['language_title'][$l['folder']])
                        ? $_POST['language_title'][$l['folder']]
                        : '';
                    $note_lang = isset($_POST['language_note'][$l['folder']])
                        ? $_POST['language_note'][$l['folder']]
                        : '';
            ...
            if ($request->ajax() == true) {
                ...
                return redirect(
                    'crud/builder/config/' . $request->input('name')
    ...
    public function postSavesetting(Request $request)
    {
        ...
        $setting = [
            ...
            'form-method' => !is_null($request->input('form-method'))
                ? $request->input('form-method')
                : 'native',
            'view-method' => !is_null($request->input('view-method'))
                ? $request->input('view-method')
                : 'native',
        ...
        \DB::table('crud')
            ->where('id', '=', $id)
            ->update([
                'config' => \CrudHelpers::CF_encode_json($new_config),
        ...
        if ($request->ajax() == true) {
            return response()->json([
                'status' => 'success',
                'message' => 'Module Setting Has Been Save Successfull',
            ]);
        } else {
            return redirect('crud/builder/config/' . $row->name)
                ->with('message', 'Module Setting Has Been Save Successfull')
                ->with('status', 'success');
        }
    }
    ...
    function postSavesql(Request $request, $id)
    {
        ...
        $total = isset($_POST['counter']) ? $_POST['counter'] : [];
        ...
        \DB::table('crud')
            ->where('id', '=', $row->id)
            ->update(['config' => \CrudHelpers::CF_encode_json($config)]);
        ...
        if ($request->ajax() == true) {
            ...
            return redirect('crud/builder/sql/' . $row->name)->with(
                'message',
                'SQL Updated'
            );
        }
    ...
    function getForm($id)
    {
        ...
        $this->data['form_column'] = isset($config['form_column'])
            ? $config['form_column']
            : 1;
        $this->data['module'] = 'module';
        $this->data['name'] = $row->name;
    ...
    public function postSaveform(Request $request)
    {
        ...
        for ($i = 1; $i <= $total; $i++) {
            ...
            foreach ($lang as $l) {
                if ($l['folder'] != 'en') {
                    $label_lang = isset($_POST['language'][$i][$l['folder']])
                        ? $_POST['language'][$i][$l['folder']]
                        : '';
            ...
            $f[] = [
                ...
                'option' => [
                    ...
                    'select_multiple' => isset($select_multiple[$i])
                        ? $select_multiple[$i]
                        : 0,
                    'image_multiple' => isset($image_multiple[$i])
                        ? $image_multiple[$i]
                        : 0,
        ...
        if ($request->ajax() == true) {
            return response()->json([
                'status' => 'success',
                'message' => 'Module Forms Has Changed Successful',
            ]);
        } else {
            return redirect('crud/builder/form/' . $row->name)
                ->with('message', 'Module Forms Has Changed Successful.')
                ->with('status', 'success');
        }
    }
            ...
            if ($form['field'] == $field_id && $form['alias'] == $alias) {
                ...
                $f = [
                    ...
                    'option' => [
                        ...
                        'select_multiple' => isset(
                            $form['option']['select_multiple']
                        )
                            ? $form['option']['select_multiple']
                            : 0,
                        'image_multiple' => isset(
                            $form['option']['image_multiple']
                        )
                            ? $form['option']['image_multiple']
                            : 0,
                        'lookup_dependency_key' =>
                            $form['option']['lookup_dependency_key'],
                        'path' => $form['option']['path'],
                        'upload_type' => $form['option']['upload_type'],
                        'resize_width' => isset($form['option']['resize_width'])
                            ? $form['option']['resize_width']
                            : '',
                        'resize_height' => isset(
                            $form['option']['resize_height']
                        )
                            ? $form['option']['resize_height']
                            : '',
                        'extend_class' => isset($form['option']['extend_class'])
                            ? $form['option']['extend_class']
                            : '',
                        'tooltip' => $tooltip,
                        'attribute' => $attribute,
                        'extend_class' => isset($form['option']['extend_class'])
                            ? $form['option']['extend_class']
                            : '',
                        'prefix' => isset($form['option']['prefix'])
                            ? $form['option']['prefix']
                            : '',
                        'sufix' => isset($form['option']['sufix'])
                            ? $form['option']['sufix']
                            : '',
                    ],
                ];
    ...
    {
        $lookup_value = is_array($request->input('lookup_value'))
            ? implode('-', array_filter($request->input('lookup_value')))
            : '';
        $id = $request->input('id');
        $row = \DB::table('crud')
            ->where('id', $id)
        ...
        $new_field = [
            ...
            'option' => [
                ...
                'select_multiple' => !is_null(
                    $request->input('select_multiple')
                )
                    ? '1'
                    : '0',
                'image_multiple' => !is_null($request->input('image_multiple'))
                    ? '1'
                    : '0',
                'lookup_dependency_key' => $request->input(
                    'lookup_dependency_key'
                ),
        ...
        foreach ($config['forms'] as $form_view) {
            if (
                $form_view['field'] == $request->input('field') &&
                $form_view['alias'] == $request->input('alias')
            ) {
                $new_form = $new_field;
            } else {
                $new_form = $form_view;
        ...
        return redirect('crud/builder/form/' . $row->name)
            ->with('message', 'Forms Has Changed Successful.')
            ->with('status', 'success');
    }
    ...
    public function postSavetable(Request $request)
    {
        ...
        for ($i = 1; $i <= $total; $i++) {
            ...
            $grid[] = [
                ...
                'format_value' => isset($format_value[$i])
                    ? $format_value[$i]
                    : '',
            ];
        }
        ...
        if ($request->ajax() == true) {
            return response()->json([
                'status' => 'success',
                'message' => 'Module Table Has Been Save Successfull',
            ]);
        } else {
            return redirect('crud/builder/table/' . $row->name)
                ->with('message', 'Module Table Has Been Save Successfull')
                ->with('status', 'success');
        }
    }
    ...
    function getPermission($id)
    {
        ...
        */
        ...
        foreach ($this->data['roles'] as $r) {
            ...
            $GA = \DB::select(
                "SELECT * FROM crud_access where id ='" . $row->id . "' $role"
            );
            if (count($GA) >= 1) {
                $GA = $GA[0];
            }

            $access_data = isset($GA->access_data)
                ? json_decode($GA->access_data, true)
                : [];
            $rows = [];
            ...
            foreach ($tasks as $item => $val) {
                $rows[$item] =
                    isset($access_data[$item]) && $access_data[$item] == 1
                        ? 1
                        : 0;
            }
            $access[$r->name] = $rows;
        ...
        $this->data['crud_access'] = \DB::select(
            "select * from crud_access where id ='" . $row->id . "'"
        );
        $this->data['type'] = $row->type == 'ajax' ? 'addon' : $row->type;
    ...
    public function postSavepermission(Request $request)
    {
        ...
        $fp =
            base_path() .
            '/resources/views/Acore/builder/template/' .
            $this->getTemplateName($row->type) .
            '/config/info.json';
        ...
        */
        ...
        if ($request->ajax() == true) {
            return response()->json([
                'status' => 'success',
                'message' => 'Permission Has Changed Successful',
            ]);
        } else {
            return redirect('crud/builder/permission/' . $row->name)
                ->with('message', 'Permission Has Changed Successful')
                ->with('status', 'success');
        }
    }
    ...
    function getFormdesign(Request $request, $id)
    {
        ...
        if (!isset($config['form_layout'])) {
            ...
            $this->data['title'] = explode(
                ',',
                $config['form_layout']['title']
            );
            $this->data['format'] = $config['form_layout']['format'];
            $this->data['display'] = isset($config['form_layout']['display'])
                ? $config['form_layout']['display']
                : 'horizontal';
        }
        $this->data['name'] = $row->name;
        $this->data['type'] = $row->type;
    ...
    public function postFormdesign(Request $request)
    {
        ...
        if ($request->ajax() == true) {
            return response()->json([
                'status' => 'success',
                'message' => 'Forms Design Has Changed Successful',
            ]);
        } else {
            return redirect('crud/builder/formdesign/' . $row->name)
                ->with('message', 'Forms Design Has Changed Successful')
                ->with('status', 'success');
        }
    }
...
*/ $this->data[
    ...
    function getSub(Request $request, $id = '')
    {
        ...
        $this->data['subs'] = isset($config['subgrid'])
            ? $config['subgrid']
            : [];
        $this->data['tables'] = Crud::getTableList($this->db);
    ...
    function postSavesub(Request $request)
    {
        ...
        if ($validator->passes()) {
            ...
            if ($request->ajax() == true) {
                return response()->json([
                    'status' => 'success',
                    'message' => 'Master Has beed added Successful.',
                ]);
            } else {
                return redirect('crud/builder/sub/' . $row->name)
                    ->with('message', 'Master Has beed added Successful.')
                    ->with('status', 'success');
            }
        } else {
    ...
    function getRemovesub(Request $request)
    {
        ...
        return redirect('crud/builder/sub/' . $row->name)
            ->with('message', 'Master Has removed Successful.')
            ->with('status', 'success');
    }
    ...
    function getConn(Request $request, $id)
    {
        ...
        foreach ($config['grid'] as $form) {
            if ($form['field'] == $field_id) {
                $f = [
                    'db' => isset($form['conn']['db'])
                        ? $form['conn']['db']
                        : '',
                    'key' => isset($form['conn']['key'])
                        ? $form['conn']['key']
                        : '',
                    'display' => isset($form['conn']['display'])
                        ? $form['conn']['display']
                        : '',
                ];
            }
            ...
            if ($form['field'] == $field_id && $form['alias'] == $alias) {
                if ($request->input('db') != '') {
                    ...
                    $form['conn'] = [
                        ...
                        'display' => implode(
                            '|',
                            array_filter($request->input('display'))
                        ),
                    ];
                } else {
                    $form['conn'] = [
    ...
    function configForm($field, $alias, $type, $sort, $opt = [])
    {
        $opt_type = '';
        $lookup_table = '';
        $lookup_key = '';
    ...
    public function getRebuild(Request $request, $id = 0)
    {
        ...
        require_once base_path() .
            '/resources/views/Acore/builder/template/native/config/config.php';
        ...
        if ($request->ajax() == true && \Auth::check() == true) {
            ...
            return redirect('crud/builder')
                ->with('message', 'Code Script has been replaced successfull')
                ->with('status', 'success');
        }
    }
    ...
    function postDopackage(Request $request)
    {
        ...
        foreach ($_modules as $n => $_module) {
            ...
            $cf_zip->add_data(
                'app/Models/' . ucwords($file) . '.php',
                file_get_contents(
                    $app_path . '/Models/' . ucwords($file) . '.php'
                )
            );
            $cf_zip->get_files_from_folder(
                "../resources/views/{$file}/",
                "resources/views/{$file}/"
            );
        }
        ...
        $this->data['download_link'] = link_to(
            "uploads/zip/{$app_name}.zip",
            'download here',
            ['target' => '_new']
        );
        ...
        return redirect('builder')
            ->with('message', ' Module(s) zipped successful ! ')
            ->with('status', 'success');
    }
    ...
    function postInstall(Request $request, $id = 0)
    {
        ...
        if ($validator->passes()) {
            ...
            return redirect('crud/builder')
                ->with('message', 'Module Installed' . $msg)
                ->with('status', 'success');
        } else {
            return redirect('crud/builder')
                ->with('message', 'Please select file to upload !')
                ->with('status', 'error');
        }
    }
    ...
    function getSubform(Request $request, $id = 0)
    {
        ...
        $this->data['row'] = $row;
        $this->data['fields'] = $config['grid'];
        $this->data['subform'] = isset($config['subform'])
            ? $config['subform']
            : [];
        if (isset($config['subform']['data'])) {
            $this->data['table_fields'] = Crud::getTableField(
                $config['subform']['table']
            );
        }
        $this->data['tables'] = Crud::getTableList($this->db);
        $this->data['module'] = $row->name;
    ...
    function postSavesubform(Request $request)
    {
        ...
        if ($validator->passes()) {
            ...
            if ($request->ajax() == true) {
                return response()->json([
                    'status' => 'success',
                    'message' => 'Subform has beed added Successful',
                ]);
            } else {
                return redirect('crud/builder/subform/' . $row->name)
                    ->with('message', 'Subform has beed added Successful.')
                    ->with('status', 'success');
            }
        } else {
            return redirect('crud/builder/subform/' . $request->get('name'))
    ...
    function getSubformremove(Request $request, $id = 0)
    {
        ...
        return redirect('crud/builder/subform/' . $row->name)
            ->with('message', 'Subform has beed Removed Successful.')
            ->with('status', 'success');
    }
            ...
            if (count($files) > 2) {
                ...
                foreach ($files as $file) {
                    if (
                        ...
                        $res .=
                            "<li class=\"file ext_$ext\"><a href=\"#\" rel=\"" .
                            htmlentities($_POST['dir'] . $file) .
                            "\">" .
                            htmlentities($file) .
                            '</a></li>';
    ...
    function getTemplate(Request $request, $id)
    {
        ...
        $this->data['attach'] =
            isset($config['template']) && $config['template'] == 'custom'
                ? 'custom'
                : 'system';

        $path = base_path() . '/resources/views/' . $row->name . '/';

        $table = file_exists($path . 'table.blade.php')
            ? $path . 'table.blade.php'
            : base_path() .
                '/resources/views/CrudEngine/default/table.blade.php';
        $form = file_exists($path . 'table.blade.php')
            ? $path . 'form.blade.php'
            : base_path() .
                '/resources/views/CrudEngine/default/form.blade.php';
        $view = file_exists($path . 'table.blade.php')
            ? $path . 'view.blade.php'
            : base_path() .
                '/resources/views/CrudEngine/default/view.blade.php';
    ...
    function attachTemplate(Request $request, $id)
    {
        ...
        return redirect('crud/builder/template/' . $row->name)->with([
            'status' => 'success',
    ...
    {
        $path =
            base_path() . '/resources/views/' . $request->input('path') . '/';
        $table = $path . 'table.blade.php';
        $request->input('table');
        $form = $path . 'form.blade.php';
        $request->input('form');
        $view = $path . 'view.blade.php';
        $request->input('view');

        $fp = fopen($table, 'w+');
        fwrite($fp, $request->input('table'));
        fclose($fp);
        $fp = fopen($form, 'w+');
        fwrite($fp, $request->input('form'));
        fclose($fp);
        $fp = fopen($view, 'w+');
        fwrite($fp, $request->input('view'));
        fclose($fp);
    ...
    public function postDuplicate(Request $request, $id)
    {
        ...
        if ($validator->passes()) {
            ...
            if (count($res) >= 1) {
                ...
                return redirect(
                    'crud/builder/rebuild/' . $row->id . '?mode=duplicate'
                );
            } else {
                return response()->json([
                    'status' => 'error',
